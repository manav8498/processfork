...
`v1.0.9` tagged. Closes the two SDK-side gaps the v1.0.8 retest flagged: the Python SDK now applies the same default secret-shaped env scrub regex that the CLI has had since v1.0.7 (so adapters that pass `dict(os.environ)` get safe-by-default redaction without each caller having to remember `--scrub-env`), and the SDK's effects ledger is now HMAC-chained through the same `pf_effects::Ledger::append` code path the CLI was wired to in v1.0.7 (so SDK-produced ledgers verify through `pf verify` and tamper / reorder / delete is detectable). All 5 first-party adapters (Claude Code, LangGraph, OpenInterpreter, AutoGen, CrewAI) inherit the safe defaults automatically. `cargo deny check`: `advisories ok, bans ok, licenses ok, sources ok`. Earlier rounds' fixes still apply (CLI scrub + ledger, vLLM/SGLang persistence, cargo-audit clear).
